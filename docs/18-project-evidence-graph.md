...
データの正本は[`project-graph.json`](graph/project-graph.json)、表示は[`index.html`](graph/index.html)と[`graph.js`](graph/graph.js)です。手書きの解説と表示ロジックを分離し、自動処理がcuratedな関係を上書きしない構造にします。
